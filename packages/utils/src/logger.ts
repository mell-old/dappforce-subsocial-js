...
prefix.apply(log, {
  format(level, name, timestamp) {
    return `${chalk.gray(`[${timestamp}]`)} ${colors[level.toUpperCase() as Levels](level.length < 5 ? `${level }` : level)} ${chalk.green(`${name}:`)}`;
  },
});

export const newLogger = (name: string) => log.getLogger(name)
